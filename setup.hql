...
CREATE TABLE message18 (
    ...
    receiver_timestamp int COMMENT "Metadata: timestamp when the message was received",
    repeat int COMMENT "how many times a message has been repeated (0-3). The number of stations received the message",
    mmsi string COMMENT "Unique number to identify the ship.",
    ...
    display boolean COMMENT "0 = No display available; not capable of displaying Message 12 and 14, 1 = Equipped with integrated display displaying Message 12 and 14",
    dsc boolean COMMENT "0 = Not equipped with DSC function, 1 = Equipped with DSC function (dedicated or time-shared)", 
    band boolean COMMENT "0 = Capable of operating over the upper 525 kHz band of the marine band, 1 = Capable of operating over the whole marine band, (irrelevant if 'Class B Message 22 flag' is 0)", 
    msg22 boolean COMMENT "0 = No frequency management via Message 22 , operating on AIS1, AIS2 only. 1 = Frequency management via Message 22", 
    assigned boolean COMMENT "0 = Station operating in autonomous and continuous mode = default. 1 = Station operating in assigned mode",
    raim boolean COMMENT "Receiver autonomous integrity monitoring (RAIM) flag of electronic position fixing device; 0 = RAIM not in use = default; 1 = RAIM in use. See Table", 
    radio string COMMENT "0 UTC direct (sync from own integral GPS receiver), 1 UTC indirect (own GPS unavailable - UTC sync from GPS, receiver on nearby ship or base station), 2 Station is synchronized to a base station (base direct - GPS unavailable), 3 Station is synchronized to another station"
    )
ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
